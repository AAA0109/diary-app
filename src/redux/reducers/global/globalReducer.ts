...
const showGlobalMessage = (state: Map<string, any>, action: any) => {
    ...
    return state.set('message', message).set('messageOpen', true);
};
...
const showMessageByReference = (state: Map<string, any>, action: any) => {
    ...
    if (refrenceKey && refrenceKey !== undefined && refrenceKey !== null) {
        return state.setIn(['messageByRef', refrenceKey], message).setIn(['messageByRefeOpen', refrenceKey], true);
    }
    return state;
};
...
const hideMessageByReference = (state: Map<string, any>, action: any) => {
    ...
    if (refrenceKey && refrenceKey !== undefined && refrenceKey !== null) {
        return state.setIn(['messageByRef', refrenceKey], '').setIn(['messageByRefeOpen', refrenceKey], false);
    }
    return state;
};
...
export const globalReducer = (state: Map<string, any> = Map(new GlobalState()), action: IGlobalAction) => {
    ...
    switch (action.type) {
        case GlobalActionType.PROGRESS_CHANGE:
            return progressChange(state, payload);

        case GlobalActionType.PROGRESS_CHANGE_WITH_KEY:
            return progressChangeWithKey(state, payload);

        case GlobalActionType.DEFAULT_DATA_DISABLE:
            return state.set('defaultLoadDataStatus', false);

        case GlobalActionType.DEFAULT_DATA_ENABLE:
            return state.set('defaultLoadDataStatus', true);

        case GlobalActionType.SHOW_MESSAGE_GLOBAL:
            return showGlobalMessage(state, action);

        case GlobalActionType.HIDE_MESSAGE_GLOBAL:
            return hideGlobalMessage(state);

        case GlobalActionType.SHOW_MESSAGE_BY_REFERENCE:
            return showMessageByReference(state, action);

        case GlobalActionType.HIDE_MESSAG_BY_REFERENCE:
            return hideMessageByReference(state, action);

        case GlobalActionType.SET_HEADER_TITLE:
            return state.set('headerTitle', action.payload);

        case GlobalActionType.SET_NAVIGATION_STEP:
            return state.set('navigationStep', action.payload);

        case GlobalActionType.SET_EMOTION:
                return state.set('emotion', action.payload);
    
        case GlobalActionType.OPEN_DIALOG:
            return state.setIn(['dialog', action.payload.type, 'open'], true);

        case GlobalActionType.CLOSE_DIALOG:
            return state.setIn(['dialog', action.payload.type, 'open'], false);

        case GlobalActionType.SHOW_SEND_FEEDBACK:
            return state.set('sendFeedbackStatus', true);

        case GlobalActionType.HIDE_SEND_FEEDBACK:
            return state.set('sendFeedbackStatus', false);
        ...
        case GlobalActionType.TEMP:
            return state.mergeIn(['temp', 'caller'], fromJS([payload.caller]));
