...
export function NotifyItemComponent(props: INotifyItemProps) {
    ...
    const { description, fullName, avatar, isRead, id, userId, url, deleteNotify } = props;
    const handleSeenNotify = (event: any) => {
        event.preventDefault();

        const { seenNotify, id, url, isRead, closeNotify } = props;
    ...
    return (
        <Button
            key={userId}
            className={classNames(classes.listItem, { [classes.notSeen]: !isRead })}
            component={'a'}
