...
import { ICreateFamilyMotoModalProps } from './ICreateFamilyMotoModalProps';
...
export function CreateFamilyMoto(props: ICreateFamilyMotoModalProps) {
    const classes = useStyles();
    const { t } = useTranslation();

    const { createFamilyMoto } = useAuth();
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setStatus, setSubmitting, resetForm }) => {
            try {
                const res = await createFamilyMoto(values.motiveHeading, values.motiveDescription);
                props.updateCurrentMoto();
                props.handleClose();
                enqueueSnackbar('Moto is created successfully', {
                    variant: 'success',
                    action: (key) => (
        ...
        >
            <BootstrapDialogTitle id="customized-dialog-title" onClose={handleClose}>
                ...
                <Typography fontSize="14px" fontWeight="400" paddingY="4px">
                    Create new goal for your family
                </Typography>
...
export default CreateFamilyMoto;
