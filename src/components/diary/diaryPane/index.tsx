...
export function DiaryPaneComponent(props: IDiaryPaneComponentProps) {
    ...
    const { user } = useAuth();
    const [isLike, setIsLike] = useState((diary.likes?.findIndex(e => e.userId?.toString() === user?.id?.toString()) ?? -1) >= 0);
    ...
    const handleLike = () => {
        if (diary?.id) {
            ...
            diaryService.likeDiary(diary.id);
        }
    }
    ...
    const editDiary = () => {
        if (!diary.id) return;
        navigate(PATH_MAIN.diary.editDiary(diary.id!));
    }

    const handleDeleteDiary = async (): Promise<boolean> => {
        if (!diary.id) return false;
        const { error } = await diaryService.deleteDiary(diary.id);
        if (error) {
        ...
        enqueueSnackbar('Diary is deleted successfully', {
            variant: 'success',
            action: (key) => (
                <IconButton size="small" onClick={() => closeSnackbar(key)}>
                    <CloseIcon />
    ...
    const handleClickSeeMore = () => {
        if (props.canExpand) {
            ...
            navigate(PATH_MAIN.diary.viewDiary(diary.id!));
        }
    }
    ...
    const handleShareDairy = () => {
        copy(`${window.location.origin}${PATH_MAIN.diary.viewSharedDiary(diary.id!, user?.password!)}`);
        enqueueSnackbar('Share link is copied to clipboard', {
            variant: 'success',
            action: (key) => (
                <IconButton size="small" onClick={() => closeSnackbar(key)}>
                    <CloseIcon />
    ...
    return (
        <Box className={`${classes.root} ${props.isFixedSize && classes.fixedSize}`}>
            <Box className={classes.header}>
                ...
                {user?.id?.toString() === diary.user?.id?.toString() && <Box>
                    <IconButton
                        aria-label="more"
                        id="long-button"
                    ...
                    <Menu
                        ...
                        onClose={handleClose}
                    >
                        <MenuItem onClick={editDiary}>
                            <EditOutlined style={{ paddingRight: "4px" }} />
                            Edit Diary
                        </MenuItem>
                        <MenuItem onClick={handleOpenDeleteModal} style={{ color: "#EB5757" }}>
                            <DeleteOutline style={{ paddingRight: "4px" }} />
                            Delete Diary
                        </MenuItem>
                    </Menu>
                </Box>}
            ...
            <Box display="flex" justifyContent="space-between" paddingY="10px">
                <Box display="flex" alignItems="center" justifyContent="space-between" gap="8px">
                    <Box display="flex" alignItems="center" justifyContent="space-between" style={{ userSelect: "none", cursor: 'pointer' }}>
                        {isLike ? <HeartIcon color="#EB5757" onClick={handleLike} /> : <HeartOutlineIcon color="#EB5757" onClick={handleLike} />}
                        {likeCount}
                    </Box>
                ...
                <ShareIcon style={{ cursor: 'pointer' }} onClick={handleShareDairy} />
            </Box>
            <Collapse in={showComments}>
                {props.showComments && CommentsComponent()}
            </Collapse>
            <ConfirmModal title="Confirm Delete" content="This data will be permanently deleted. Do you confirm that?" confirmText="Delete" open={Boolean(showDeleteComment)} handleClose={() => setShowDeleteComment(null)} confirmIcon={<DeleteOutline style={{ marginRight: '4px' }} />} handleConfirm={handleDeleteChat} />
            <ConfirmDeleteModal open={deleteModalOpen} handleClose={() => setDeleteModalOpen(false)} handleConfirm={handleDeleteDiary} />
        </Box>
    );
}
...
export default DiaryPaneComponent;
